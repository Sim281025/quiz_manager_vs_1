...
//router.get('/', auth, role, aync (req, res) => {
router.get('/', auth, async (req, res) => { //pull from db
    
    try {
        const quizzes = await Quiz.find({ user: req.user.id }).sort({ date: -1 }); //finds the user objectID from db and sorts the array into date order of most recent first
        res.json(quizzes);
    } catch (err) {
...
router.post('/', 
    [auth, 
    authRole('admin'),[
        check('quizTitle', 'Quiz title is required')
            .not()
            .isEmpty()
    ]
    ], 
